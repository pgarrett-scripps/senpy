...
from ms2_extractor import extract_ms2_file

RAW_CONVERTOR_PATH = Path("ms2_extractor.py")


class SelectFolders(QFileDialog):
    def __init__(self, *args):
        super(SelectFolders, self).__init__(*args)
...
class RawFileTinker(Frame):
    def __init__(self):
        ...
        self.button = Button(self.folder_frame, text="Select D folders", command=self.load_file,
                             width=int(self._frame_width / 3))
        self.button.pack(side=LEFT, expand=True)

        self.overwrite_flag = BooleanVar()
        ...
        self.timsscore_flag = BooleanVar()
        self.timsscore_button = ttk.Checkbutton(self.folder_frame,
                                                text='timsscore format',
                                                variable=self.timsscore_flag,
                                                onvalue=True,
                                                offvalue=False)
        self.timsscore_button.pack(side=RIGHT, expand=True)

        self.mobility_spectra_flag = BooleanVar()
        self.mobility_spectra_flag_button = ttk.Checkbutton(self.folder_frame,
                                                            text='include mobility spectrum',
                                                            variable=self.mobility_spectra_flag,
                                                            onvalue=True,
                                                            offvalue=False)
        self.mobility_spectra_flag_button.pack(side=RIGHT, expand=True)

        self.d_folder_label = Label(self, text="no file selected", width=self._frame_width, height=2)
        self.d_folder_label.pack(expand=True)
    ...
    def generate_ms2s(self):
        ...
        for i, folder in enumerate(self.raw_folders):
            ...
            if contains_file and not self.overwrite_flag.get():
                ...
                extract_ms2_file(analysis_dir=folder,
                                 output_file=None,
                                 include_mobility_spectra=self.mobility_spectra_flag.get(),
                                 ppm=15,
                                 skip_spectra=False
                                 )
    ...
    def load_file(self):
        ...
        self.d_folder_label['text'] = f'Files selected: {number_files_selected}'
        self.button_ms1['state'] = NORMAL
        self.button_ms2['state'] = NORMAL
